...
// *** NEW (A): Helper function for "Must Play" check ***
function checkIfPlayerMustPlay(player, topCard, activeColor) {
    if (!player || !player.hand || player.hand.length === 0) {
        return false;
    }
    // Note: This check assumes drawPenalty is 0, as it's for voluntary draws.
    for (const card of player.hand) {
        if (card.color !== 'Black') { // Skip wild cards
            if (card.color === activeColor || card.value === topCard.value) {
                return true; // Found a playable non-wild card
            }
        }
    }
    return false; // No playable non-wild cards
}
...
io.on('connection', (socket) => {
  ...
  function checkAndStartNextRound() {
    ...
    if (hostIsReady && allPlayersReady) {
        ...
        // Skip disconnected players for dealing START
        let maxAttempts = gameState.players.length;
        while (gameState.players[newDealerIndex].status !== 'Active' && maxAttempts > 0) {
            addLog(`Dealer ${gameState.players[newDealerIndex].name} is not active. Skipping.`);
            newDealerIndex = (newDealerIndex + 1) % gameState.players.length;
            maxAttempts--;
        }
        if (gameState.players[newDealerIndex].status !== 'Active') {
             addLog("Error: No active player found to be the next dealer!");
             return; // Stay in RoundOver phase
        } // Skip disconnected players for dealing END
        gameState.dealerIndex = newDealerIndex;
        const dealer = gameState.players[newDealerIndex];
        gameState.playerChoosingActionId = dealer.playerId;
  ...
  socket.on('playerReadyForNextRound', () => {
      ...
      // Allow 'Active' players to ready up
      if (player && player.status === 'Active' && !gameState.readyForNextRound.includes(player.playerId)) {
          gameState.readyForNextRound.push(player.playerId);
          checkAndStartNextRound(); // Check if this triggers the transition
          io.emit('updateGameState', gameState); // Update UI
      }
  });
  ...
  socket.on('endGame', () => { if(gameState && gameState.phase !== 'GameOver') { const player = gameState.players.find(p => p.socketId === socket.id); if (player && player.isHost) { gameState.phase = 'GameOver'; io.emit('finalGameOver', gameState); addLog(`The game has been ended by the host.`); gameState = null; players = []; reconnectTimers = {}; } } });
  ...
  socket.on('drawCard', () => { 
    ...
    if (playerIndex === gameState.currentPlayerIndex) { 
        ...
        if (gameState.pickUntilState?.active && gameState.pickUntilState.targetPlayerIndex === playerIndex) { 
            ...
            // *** MODIFIED (A): Added server-side "Must Play" check ***
        ...
        if (gameState.phase !== 'GameOver' && gameState.phase !== 'RoundOver' ) { 
            io.emit('updateGameState', gameState); 
        } 
    } 
  });
  ...
  socket.on('disconnect', () => { console.log(`Player disconnected: ${socket.id}`); if (gameState && gameState.phase !== 'GameOver') { const disconnectedPlayer = gameState.players.find(p => p.socketId === socket.id); if (disconnectedPlayer && disconnectedPlayer.status === 'Active') { disconnectedPlayer.status = 'Disconnected'; addLog(`Player ${disconnectedPlayer.name} has disconnected. The game is paused.`); gameState.isPaused = true; const pauseEndTime = Date.now() + DISCONNECT_GRACE_PERIOD; const allDisconnected = gameState.players.filter(p => p.status === 'Disconnected'); gameState.pauseInfo = { pauseEndTime: pauseEndTime, pausedForPlayerNames: allDisconnected.map(p => p.name) }; if (reconnectTimers[disconnectedPlayer.playerId]) clearTimeout(reconnectTimers[disconnectedPlayer.playerId]); reconnectTimers[disconnectedPlayer.playerId] = setTimeout(() => handlePlayerRemoval(disconnectedPlayer.playerId), DISCONNECT_GRACE_PERIOD); io.emit('updateGameState', gameState); } } else { const playerInLobby = players.find(player => player.socketId === socket.id); if (playerInLobby) { players = players.filter(player => player.socketId !== socket.id); if (playerInLobby.isHost && players.length > 0) { players[0].isHost = true; } io.emit('lobbyUpdate', players); } } });
