...
function startNewRound(gs) {
    ...
    addLog(`First card: ${topCard.color !== 'Black' ? topCard.color + ' ' : ''}${topCard.value}`); // Log the first card

    let firstPlayerIndex = (gs.dealerIndex + 1) % numPlayers;
    while (gs.players[firstPlayerIndex].status !== 'Active') { firstPlayerIndex = (firstPlayerIndex + 1) % numPlayers; if (firstPlayerIndex === (gs.dealerIndex + 1) % numPlayers) break; }
    ...
    if (topCard.color !== 'Black') {
        ...
        gs.discardPile[0].playerName = dealer.name; // Dealer "played" the wild
...
io.on('connection', (socket) => {
  ...
  socket.on('startGame', ({ password }) => {
      ...
      const readyPlayers = activePlayers.filter(p => p.isReady); // Get ready players

      if (readyPlayers.length < 2) { // Check count of ready players
          return socket.emit('announce', 'Need at least 2 ready players (including host) to start.');
      }

      // Start the game with only ready players
      ...
      io.emit('updateGameState', gameState); // Only ready players will receive this correctly
  });
  ...
  // *** MODIFIED: hardReset removes reconnectTimers reference ***
  socket.on('hardReset', () => {
      ...
      if (host) {
          ...
          gameState = null;
          // Object.keys(reconnectTimers).forEach(key => clearTimeout(reconnectTimers[key])); // REMOVED
          // reconnectTimers = {}; // REMOVED
          ...
          if (hostData) {
            ...
            players = [];
            console.error("Could not find host data during hard reset!");
          }
  ...
  // *** MODIFIED: Add unoDeclarationSuccess emit ***
  socket.on('callUno', () => {
    if (!gameState || gameState.phase !== 'Playing' || gameState.isPaused) return;
    const player = gameState.players.find(p => p.socketId === socket.id);
    // Check if it's the player's turn and they have exactly 2 cards
    if (player && player.hand.length === 2 && gameState.players[gameState.currentPlayerIndex].playerId === player.playerId) {
        ...
        socket.emit('unoDeclarationSuccess'); // Emit confirmation back to player
        io.emit('updateGameState', gameState); // Update everyone else
    } else if (player) {
         // Optionally send a failure message if they call at the wrong time
         // socket.emit('announce', 'You can only declare UNO when you have 2 cards on your turn.');
    }
  });


  // *** MODIFIED: Auto-UNO Call Fix ***
  socket.on('drawCard', () => {
            ...
            if (gameState.drawPile.length > 0) {
                ...
                if (isMoveValid(drawnCard, topCard, gameState.activeColor, 0)) {
                    if (drawnCard.color === 'Black') { // Drawn Wild Card
                        ...
                        // Emit privately BEFORE global update ensures modal appears first
                        io.to(socket.id).emit('drawnWildCard', { cardIndex, drawnCard });
                        io.emit('updateGameState', gameState); // Update state so others see card count change
                        ...
                        if (player.hand.length === 1) {
                            player.unoState = 'declared'; // Mark declared
                            io.emit('unoCalled', { playerName: player.name }); // Announce UNO
                            addLog(`📣 ${player.name} reached UNO automatically after drawing.`);
                        } else {
                            player.unoState = 'safe'; // Only set safe if > 1 card
                        }
                        // -------------------------
                        ...
                        if (drawnCard.value === 'Skip' || (drawnCard.value === 'Reverse' && numActivePlayers === 2)) {
                            advanceTurn(); // Extra advance for skip/reverse
                        }
                        advanceTurn(); // Normal turn advance
                        gameState.phase = 'Playing';
                    }
                } else { // Drawn Non-Playable Card
                    player.hand.push(drawnCard);
