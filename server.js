...
let gameState = null;
// let reconnectTimers = {}; // REMOVED
let globalPauseTimeout = null; // NEW: Single timer for pause duration
let gameOverToLobbyTimer = null;
let scoresToLobbyTimer = null;
...
function createDeck() { /* ... (unchanged) ... */ const deck = []; const colors = ['Red', 'Green', 'Blue', 'Yellow']; const values = ['1', '2', '3', '4', '5', '6', '7', '8', '9', 'Skip', 'Reverse', 'Draw Two']; for (const color of colors) { deck.push({ color, value: '0' }); for (let i = 0; i < 2; i++) { for (const value of values) { deck.push({ color, value }); } } } for (let i = 0; i < 4; i++) { deck.push({ color: 'Black', value: 'Wild' }); deck.push({ color: 'Black', value: 'Wild Draw Four' }); deck.push({ color: 'Black', value: 'Wild Pick Until' }); } deck.push({ color: 'Black', value: 'Wild Swap' }); return deck; }
function calculateScore(hand) { /* ... (unchanged) ... */ let score = 0; hand.forEach(card => { if (!isNaN(card.value)) { score += parseInt(card.value); } else { switch(card.value) { case 'Wild Swap': score += 100; break; case 'Draw Two': score += 25; break; case 'Skip': case 'Reverse': score += 20; break; default: score += 50; break; } } }); return score; }
function shuffleDeck(deck) { /* ... (unchanged) ... */ for (let i = deck.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [deck[i], deck[j]] = [deck[j], deck[i]]; } return deck; }

function setupGame(lobbyPlayers) { /* ... (unchanged) ... */
...
function handlePlayerRemoval(playerId) {
    ...
    if (player && player.status === 'Disconnected') {
        ...
        if (activePlayers.length < 2 && gameState.phase !== 'GameOver') {
            addLog('Less than 2 active players remaining. Game over.');
            gameState.phase = 'GameOver';
            // Determine winner based on score before removing player? Or just end? Let's just end for now.
             const finalGamePlayers = [...gameState.players];
             const lowestScore = Math.min(...finalGamePlayers.filter(p => p.status !== 'Removed').map(p => p.score));
             const winners = finalGamePlayers.filter(p => p.status !== 'Removed' && p.score === lowestScore);
             const winnerNames = winners.map(w => w.name).join(' and ');
             io.emit('announceFinalWinner', { winnerNames });
             // Short delay then show scores
             setTimeout(() => {
                if(gameState) io.emit('finalGameOver', gameState); // Emit final state before potential lobby transition
                 // Set short timer to go back to lobby
                setTimeout(() => {
                     players = finalGamePlayers
                        .filter(p => p.status !== 'Removed')
                        .map(p => ({ playerId: p.playerId, socketId: p.socketId, name: p.name, isHost: p.isHost, isReady: p.isHost, active: true }));
                     const hostExists = players.some(p => p.isHost);
                     if (!hostExists && players.length > 0) { players[0].isHost = true; players[0].isReady = true; }
                     else if (hostExists) { const host = players.find(p=>p.isHost); if(host) host.isReady = true; }
                     gameState = null;
                     io.emit('lobbyUpdate', players);
                }, 5000); // Shorter delay after scores in this case
             }, 3000);
            return; // Stop further processing for this removal
        }

        // Check if removing this player resolves the pause state
        const remainingDisconnected = gameState.players.filter(p => p.status === 'Disconnected');
        if (remainingDisconnected.length === 0 && gameState.isPaused) {
            // This case might be less likely if removal happens strictly after timer
            // but keep for robustness
            gameState.isPaused = false;
            gameState.pauseInfo = { pauseEndTime: null, pausedForPlayerNames: [] };
             if (globalPauseTimeout) { clearTimeout(globalPauseTimeout); globalPauseTimeout = null; } // Clear timer too
            addLog("Last disconnected player removed by timer. Game resumed.");
        } else if (gameState.isPaused) {
            // Update names if pause continues
            gameState.pauseInfo.pausedForPlayerNames = remainingDisconnected.map(p => p.name);
        }

        // Advance turn if the removed player was the current player
        const currentActivePlayer = gameState.players[gameState.currentPlayerIndex];
        if (['Playing', 'ChoosingColor', 'ChoosingPickUntilAction', 'ChoosingSwapHands'].includes(gameState.phase) && currentActivePlayer?.playerId === playerId) {
            ...
            if (gameState.playerChoosingActionId === playerId) {
                ...
                gameState.phase = 'Playing'; // Revert phase if chooser was removed
            }
            advanceTurn();
        }

        io.emit('updateGameState', gameState); // Update clients with removal/state changes
    } else {
         console.log(`handlePlayerRemoval called for ${playerId}, but player was not found or not Disconnected.`);
    }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('joinGame', ({ playerName, playerId }) => {
    ...
    if (gameState && gameState.phase !== 'Lobby' && gameState.phase !== 'GameOver') {
        ...
        if (playerId) {
            ...
            console.log(`Attempting ID match for ${playerId}: ${playerToRejoin ? 'Found' : 'Not Found'}`);
        }
        ...
        if (!playerToRejoin && disconnectedPlayers.length > 0) {
            ...
            console.log(`Attempting Name match for ${playerName}: ${playerToRejoin ? 'Found' : 'Not Found'}`);
        }

        if (playerToRejoin) {
            console.log(`Player ${playerName} (${playerId}) rejoining as ${playerToRejoin.name}.`);
            // clearTimeout(reconnectTimers[playerToRejoin.playerId]); // REMOVED
            // delete reconnectTimers[playerToRejoin.playerId]; // REMOVED
            ...
            if (stillDisconnected.length === 0) {
                gameState.isPaused = false;
                gameState.pauseInfo = { pauseEndTime: null, pausedForPlayerNames: [] };
                if (globalPauseTimeout) { // Clear the global timer if everyone is back
                ...
                // Update names, timer continues based on last disconnect
            ...
            socket.emit('joinSuccess', { playerId: playerToRejoin.playerId, lobby: players }); // Send lobby for context initially? No, game state below.
            io.emit('updateGameState', gameState); // Send full state including reconnected player
            return;
        } else {
  ...
  socket.on('markPlayerAFK', ({ playerIdToMark }) => {
      ...
      if (host && playerToMark && playerToMark.status === 'Active') {
          ...
          addLog(`Host ${host.name} marked ${playerToMark.name} as AFK. Game pause timer updated/started.`);
          ...
          globalPauseTimeout = setTimeout(() => {
              if (gameState && gameState.isPaused && Date.now() >= gameState.pauseInfo.pauseEndTime) {
                  ...
                  // Re-check pause state after removals
                   if (gameState && !gameState.players.some(p => p.status === 'Disconnected')) {
                       ...
                       io.emit('updateGameState', gameState); // Emit if resumed
                   }
              }
              globalPauseTimeout = null;
  ...
  socket.on('playerIsBack', () => {
      if (!gameState || gameState.phase === 'GameOver') return; // Allow even if not paused? Yes.
      ...
      if (player && player.status === 'Disconnected') {
          ...
          const stillDisconnected = gameState.players.filter(p => p.status === 'Disconnected');
          if (stillDisconnected.length === 0 && gameState.isPaused) {
              gameState.isPaused = false;
              gameState.pauseInfo = { pauseEndTime: null, pausedForPlayerNames: [] };
              if (globalPauseTimeout) { // Clear timer if everyone is back
                  clearTimeout(globalPauseTimeout);
                  globalPauseTimeout = null;
                  console.log("All players back, clearing global pause timer.");
              }
              addLog("All players are back. Game resumed.");
          } else if (gameState.isPaused) {
              // Update names, timer continues
              gameState.pauseInfo.pausedForPlayerNames = stillDisconnected.map(p => p.name);
               console.log(`Player ${player.name} back, but others still disconnected. Timer continues.`);
          }
  ...
  socket.on('disconnect', () => {
    ...
    if (gameState && gameState.phase !== 'GameOver') {
        ...
        // Ensure player exists and was active before disconnecting
        if (disconnectedPlayer && disconnectedPlayer.status === 'Active') {
            ...
            if (globalPauseTimeout) clearTimeout(globalPauseTimeout);
            globalPauseTimeout = setTimeout(() => {
                // Check gameState still exists (wasn't hard reset) and timer actually expired
                if (gameState && gameState.isPaused && Date.now() >= gameState.pauseInfo.pauseEndTime) {
                    console.log(`Global pause timer expired at ${new Date()}. Checking for removals.`);
                    const playersToRemove = gameState.players.filter(p => p.status === 'Disconnected');
                    playersToRemove.forEach(p => { handlePlayerRemoval(p.playerId); });
                    // Re-check pause state after removals (in case some remain but timer ended)
                    if (gameState && !gameState.players.some(p => p.status === 'Disconnected')) {
                       gameState.isPaused = false;
                       gameState.pauseInfo = { pauseEndTime: null, pausedForPlayerNames: [] };
                       console.log("All disconnected players removed by timer, resuming game.");
