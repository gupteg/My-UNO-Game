...
function setupGame(lobbyPlayers) {
    ...
    return {
        ...
        pickUntilState: null, // { chooserPlayerId, targetPlayerIndex, active, targetColor }
        swapState: null, // { choosingPlayerId } // Simplified
    };
}
...
function startNewRound(gs) {
    ...
    if (topCard.color !== 'Black') {
        ...
        if (topCard.value === 'Reverse') {
            if (connectedPlayersCount > 2) {
                ...
                let tempIndex = gs.dealerIndex;
                 do { tempIndex = (tempIndex - 1 + numPlayers) % numPlayers; }
        ...
        gs.phase = 'Playing'; // *** Start playing ***
        ...
        if (topCard.value === 'Wild Pick Until') {
             gs.phase = 'ChoosingPickUntilAction'; // *** Go to Pick Until choice ***
        } else {
             gs.phase = 'ChoosingColor'; // *** Go directly to Color choice ***
        }
    }
    return gs;
}
...
function advanceTurn() {
    ...
    if (activePlayers.length === 0) { addLog("No active players left to advance turn."); return; }
    const currentPlayer = gameState.players[gameState.currentPlayerIndex];
    if (currentPlayer && currentPlayer.unoState === 'declared') { currentPlayer.unoState = 'safe'; }
    do {
...
function handleEndOfRound(winners) {
    if (gameState.phase === 'RoundOver' || gameState.phase === 'GameOver') return;
    gameState.phase = 'RoundOver'; // *** Set phase ***
    ...
    if (isMoveValid(playedCard, topCard, gameState.activeColor, gameState.drawPenalty)) {
        ...
        if (player.hand.length === 0) {
            if (actionCardsThatDelayWin.includes(playedCard.value)) {
                ...
                handleEndOfRound([player]); return; // Round over
            }
        }
        ...
        if (playedCard.color === 'Black') {
            ...
            switch (playedCard.value) {
                case 'Wild Pick Until':
                    gameState.phase = 'ChoosingPickUntilAction'; break;
                case 'Wild Swap':
                    gameState.phase = 'ChoosingColor';
                    gameState.swapState = { choosingPlayerId: player.playerId }; break;
                default: // Wild, Wild Draw Four
                    gameState.phase = 'ChoosingColor'; break;
            }
            // *** Apply penalty ONLY here for WD4 ***
            ...
            applyCardEffect(playedCard); // Apply D2, Reverse (direction)
...
function handlePlayerRemoval(playerId) {
    ...
    if (player && player.status === 'Disconnected') {
        ...
        if (activePlayers.length < 2 && gameState.phase !== 'GameOver') {
            ...
            gameState = null; reconnectTimers = {}; return;
        }
        ...
        if (remainingDisconnected.length === 0 && gameState.isPaused) {
            gameState.isPaused = false; gameState.pauseInfo = { pauseEndTime: null, pausedForPlayerNames: [] };
        ...
        if (['Playing', 'ChoosingColor', 'ChoosingPickUntilAction', 'ChoosingSwapHands'].includes(gameState.phase) && currentActivePlayer?.playerId === playerId) {
            ...
            if (gameState.playerChoosingActionId === playerId) {
                 gameState.playerChoosingActionId = null; gameState.phase = 'Playing';
            }
            advanceTurn();
        }
...
io.on('connection', (socket) => {
  ...
  socket.on('joinGame', ({ playerName, playerId }) => {
    if (gameState && gameState.phase !== 'Lobby' && gameState.phase !== 'GameOver') {
        ...
        if (playerId) { playerToRejoin = gameState.players.find(p => p.playerId === playerId && p.status !== 'Active'); }
        ...
        if (playerToRejoin) {
            ...
            playerToRejoin.status = 'Active'; playerToRejoin.socketId = socket.id; playerToRejoin.name = playerName;
            if (reconnectTimers[playerToRejoin.playerId]) { clearTimeout(reconnectTimers[playerToRejoin.playerId]); delete reconnectTimers[playerToRejoin.playerId]; }
            ...
            if (otherDisconnected.length === 0 && gameState.isPaused) {
                gameState.isPaused = false; gameState.pauseInfo = { pauseEndTime: null, pausedForPlayerNames: [] }; addLog("All players reconnected. Game resumed.");
            } else if (gameState.isPaused) { gameState.pauseInfo.pausedForPlayerNames = otherDisconnected.map(p => p.name); }
            socket.emit('joinSuccess', { playerId: playerToRejoin.playerId, lobby: gameState.players });
            io.emit('updateGameState', gameState); return;
        } else { socket.emit('announce', 'Game is in progress. Cannot join now.'); return; }
    }
    // --- LOBBY JOIN LOGIC ---
    let pId = playerId; if (!pId) pId = Math.random().toString(36).substr(2, 9);
    const existingPlayer = players.find(p => p.playerId === pId);
    if (existingPlayer) { existingPlayer.socketId = socket.id; existingPlayer.name = playerName; }
    else { const isHost = players.length === 0; players.push({ playerId: pId, socketId: socket.id, name: playerName, isHost }); }
    socket.emit('joinSuccess', { playerId: pId, lobby: players }); io.emit('lobbyUpdate', players);
  });

  socket.on('kickPlayer', ({ playerIdToKick }) => { /* ... (unchanged) ... */ if (gameState && gameState.phase !== 'Lobby') return; const host = players.find(p => p.socketId === socket.id && p.isHost); if (host) { const playerToKick = players.find(p => p.playerId === playerIdToKick); if (playerToKick) { console.log(`Host ${host.name} kicked ${playerToKick.name}`); players = players.filter(player => player.playerId !== playerIdToKick); io.emit('lobbyUpdate', players); } } });

  socket.on('startGame', () => { /* ... (unchanged, sets phase to Dealing) ... */ if (gameState && gameState.phase !== 'Lobby') return; const host = players.find(p => p.socketId === socket.id && p.isHost); if (host && players.length >= 2) { gameState = setupGame(players); const newDealerIndex = (gameState.dealerIndex + 1) % gameState.players.length; gameState.dealerIndex = newDealerIndex; gameState.playerChoosingActionId = gameState.players[newDealerIndex].playerId; gameState.phase = 'Dealing'; players = []; io.emit('updateGameState', gameState); } });

  function checkAndStartNextRound() { /* ... (unchanged, sets phase to Dealing) ... */ if (!gameState || gameState.phase !== 'RoundOver') return; const host = gameState.players.find(p => p.isHost); const connectedPlayers = gameState.players.filter(p => p.status === 'Active'); if (!host) return; const hostIsReady = gameState.readyForNextRound.includes(host.playerId); const allPlayersReady = gameState.readyForNextRound.length === connectedPlayers.length; if (hostIsReady && allPlayersReady) { const newDealerIndex = (gameState.dealerIndex + 1) % gameState.players.length; gameState.dealerIndex = newDealerIndex; gameState.playerChoosingActionId = gameState.players[newDealerIndex].playerId; gameState.phase = 'Dealing'; io.emit('updateGameState', gameState); } }
  ...
  socket.on('dealChoice', ({ numCards }) => { /* ... (unchanged, startNewRound sets next phase) ... */ if (!gameState || gameState.phase !== 'Dealing' || gameState.isPaused) return; const dealingPlayer = gameState.players.find(p => p.socketId === socket.id); if (gameState.playerChoosingActionId === dealingPlayer?.playerId) { const numToDeal = Math.max(1, Math.min(13, parseInt(numCards) || 7)); gameState.numCardsToDeal = numToDeal; gameState.playerChoosingActionId = null; gameState = startNewRound(gameState); io.emit('updateGameState', gameState); } });
  socket.on('endGame', () => { /* ... (unchanged, sets phase to GameOver) ... */ if(gameState && gameState.phase !== 'GameOver') { const player = gameState.players.find(p => p.socketId === socket.id); if (player && player.isHost) { gameState.phase = 'GameOver'; io.emit('finalGameOver', gameState); addLog(`The game has been ended by the host.`); gameState = null; players = []; reconnectTimers = {}; } } });
  socket.on('playCard', ({ cardIndex }) => { /* ... (unchanged, relies on handleCardPlay) ... */ if (!gameState || gameState.phase !== 'Playing' || gameState.isPaused) return; const playerIndex = gameState.players.findIndex(p => p.socketId === socket.id); if (playerIndex !== -1) { handleCardPlay(playerIndex, cardIndex); if (gameState && gameState.phase !== 'RoundOver' && gameState.phase !== 'GameOver') { io.emit('updateGameState', gameState); } } });
  socket.on('callUno', () => { /* ... (unchanged, added turn check) ... */ if (!gameState || gameState.phase !== 'Playing' || gameState.isPaused) return; const player = gameState.players.find(p => p.socketId === socket.id); if (player && player.hand.length === 2 && gameState.players[gameState.currentPlayerIndex].playerId === player.playerId) { player.unoState = 'declared'; addLog(`📣 ${player.name} is ready to call UNO!`); socket.emit('announce', 'UNO declared!'); } });
  ...
  socket.on('drawCard', () => {
    ...
    if (playerIndex === gameState.currentPlayerIndex) {
        ...
        if (gameState.pickUntilState?.active && gameState.pickUntilState.targetPlayerIndex === playerIndex) {
            if (gameState.drawPile.length > 0) { /* ... (Pick Until Draw Logic - unchanged) ... */
                const drawnCard = gameState.drawPile.shift(); player.hand.push(drawnCard); io.emit('animateDraw', { playerId: player.playerId, count: 1 }); addLog(`› ${player.name} is picking for a ${gameState.pickUntilState.targetColor}...`); if (drawnCard.color === gameState.pickUntilState.targetColor) { player.hand.splice(player.hand.findIndex(c => c === drawnCard), 1); gameState.discardPile.unshift({ card: drawnCard, playerName: player.name }); gameState.activeColor = drawnCard.color; io.to(socket.id).emit('announce', `You drew the target color (${drawnCard.value} ${drawnCard.color}) and it was played for you.`); addLog(`› ${player.name} found and played a ${drawnCard.color} card.`); const pickUntilChooserId = gameState.pickUntilState.chooserPlayerId; gameState.pickUntilState = null; if (player.hand.length === 0) { const heldWinners = gameState.players.filter(p => gameState.winnerOnHold.includes(p.playerId)); handleEndOfRound([player, ...heldWinners]); return; } if (gameState.winnerOnHold.includes(pickUntilChooserId)) { const heldWinners = gameState.players.filter(p => gameState.winnerOnHold.includes(p.playerId)); handleEndOfRound(heldWinners); return; } applyCardEffect(drawnCard); const numActivePlayers = gameState.players.filter(p => p.status === 'Active').length; if (drawnCard.value === 'Skip' || (drawnCard.value === 'Reverse' && numActivePlayers === 2)) { advanceTurn(); } advanceTurn(); gameState.phase = 'Playing'; } else { player.unoState = 'safe'; }
            } else { addLog(`Draw pile empty! ${player.name} couldn't find the color.`); gameState.pickUntilState = null; advanceTurn(); gameState.phase = 'Playing'; }
        }
        // Handle Draw Penalty draw
        else if (gameState.drawPenalty > 0) { /* ... (Draw Penalty Logic - unchanged) ... */
             const penalty = gameState.drawPenalty; for (let i = 0; i < penalty; i++) { if (gameState.drawPile.length > 0) player.hand.push(gameState.drawPile.shift()); } io.emit('animateDraw', { playerId: player.playerId, count: penalty }); addLog(`› ${player.name} drew ${penalty} cards.`); player.unoState = 'safe'; gameState.drawPenalty = 0; if (gameState.winnerOnHold.length > 0) { const heldWinners = gameState.players.filter(p => gameState.winnerOnHold.includes(p.playerId)); if (!heldWinners.some(w => w.playerId === player.playerId)) { handleEndOfRound(heldWinners); return; } else { gameState.winnerOnHold = []; } } advanceTurn(); gameState.phase = 'Playing';
        }
        // Handle Regular draw
        else {
  ...
  socket.on('colorChosen', ({ color }) => {
    ...
    if (gameState.swapState) { // For Wild Swap
        gameState.phase = 'ChoosingSwapHands'; // Transition phase
        // playerChoosingActionId remains
    } else if (gameState.pickUntilState) { // For Pick Until
        gameState.pickUntilState.active = true;
        gameState.pickUntilState.targetColor = color;
        gameState.currentPlayerIndex = gameState.pickUntilState.targetPlayerIndex; // Target player's turn
        ...
        gameState.phase = 'Playing'; // Phase is playing, draw logic checks state
        gameState.playerChoosingActionId = null; // Clear chooser
    } else { // For regular Wild / WD4 / or initial dealer Wild
        let announceMsg = '';
        const playedCard = gameState.discardPile[0]?.card; // Get the card that triggered this

        // *** FIX: Construct specific announcement ***
        if (!wasDealerChoosingFirstCard && playedCard) {
             if (playedCard.value === 'Wild Draw Four') {
                 announceMsg = `✨ ${choosingPlayer.name} played Wild Draw Four and chose ${color}.`;
             } else if (playedCard.value === 'Wild') {
                 announceMsg = `✨ ${choosingPlayer.name} played Wild and chose ${color}.`;
             } else {
                 // Fallback for unexpected card, like Wild Pick Until discard path
                 announceMsg = `✨ ${choosingPlayer.name} chose ${color}.`;
             }
             io.emit('announce', announceMsg);
        }

        // *** FIX: Apply WD4 effect ONLY if applicable and NOT already applied ***
        // Penalty is now applied in handleCardPlay, so no applyCardEffect here.

        if (!wasDealerChoosingFirstCard) {
             advanceTurn();
        }
        gameState.phase = 'Playing'; // Set phase
        gameState.playerChoosingActionId = null; // Clear chooser
    }
    io.emit('updateGameState', gameState);
  });
  ...
  socket.on('markPlayerAFK', ({ playerIdToMark }) => { /* ... (unchanged) ... */ if (!gameState || ['Lobby', 'GameOver'].includes(gameState.phase) || gameState.isPaused) return; const host = gameState.players.find(p => p.socketId === socket.id && p.isHost); const playerToMark = gameState.players.find(p => p.playerId === playerIdToMark); if (host && playerToMark && playerToMark.status === 'Active') { playerToMark.status = 'Disconnected'; addLog(`Host ${host.name} marked ${playerToMark.name} as AFK. The game is paused.`); gameState.isPaused = true; const pauseEndTime = Date.now() + DISCONNECT_GRACE_PERIOD; const allDisconnected = gameState.players.filter(p => p.status === 'Disconnected'); gameState.pauseInfo = { pauseEndTime: pauseEndTime, pausedForPlayerNames: allDisconnected.map(p => p.name) }; if (reconnectTimers[playerToMark.playerId]) clearTimeout(reconnectTimers[playerToMark.playerId]); reconnectTimers[playerToMark.playerId] = setTimeout(() => handlePlayerRemoval(playerToMark.playerId), DISCONNECT_GRACE_PERIOD); io.to(playerToMark.socketId).emit('youWereMarkedAFK'); io.emit('updateGameState', gameState); } });
  socket.on('playerIsBack', () => { /* ... (unchanged) ... */ if (!gameState || gameState.phase === 'GameOver') return; const player = gameState.players.find(p => p.socketId === socket.id); if (player && player.status === 'Disconnected') { player.status = 'Active'; addLog(`Player ${player.name} is back!`); if (reconnectTimers[player.playerId]) { clearTimeout(reconnectTimers[player.playerId]); delete reconnectTimers[player.playerId]; } const otherDisconnected = gameState.players.filter(p => p.status === 'Disconnected'); if (otherDisconnected.length === 0 && gameState.isPaused) { gameState.isPaused = false; gameState.pauseInfo = { pauseEndTime: null, pausedForPlayerNames: [] }; addLog("All players are back. Game resumed."); } else if (gameState.isPaused) { gameState.pauseInfo.pausedForPlayerNames = otherDisconnected.map(p => p.name); } io.emit('updateGameState', gameState); } });
  socket.on('disconnect', () => { /* ... (unchanged) ... */ console.log(`Player disconnected: ${socket.id}`); if (gameState && gameState.phase !== 'GameOver') { const disconnectedPlayer = gameState.players.find(p => p.socketId === socket.id); if (disconnectedPlayer && disconnectedPlayer.status === 'Active') { disconnectedPlayer.status = 'Disconnected'; addLog(`Player ${disconnectedPlayer.name} has disconnected. The game is paused.`); gameState.isPaused = true; const pauseEndTime = Date.now() + DISCONNECT_GRACE_PERIOD; const allDisconnected = gameState.players.filter(p => p.status === 'Disconnected'); gameState.pauseInfo = { pauseEndTime: pauseEndTime, pausedForPlayerNames: allDisconnected.map(p => p.name) }; if (reconnectTimers[disconnectedPlayer.playerId]) clearTimeout(reconnectTimers[disconnectedPlayer.playerId]); reconnectTimers[disconnectedPlayer.playerId] = setTimeout(() => handlePlayerRemoval(disconnectedPlayer.playerId), DISCONNECT_GRACE_PERIOD); io.emit('updateGameState', gameState); } } else { const playerInLobby = players.find(player => player.socketId === socket.id); if (playerInLobby) { players = players.filter(player => player.socketId !== socket.id); if (playerInLobby.isHost && players.length > 0) { players[0].isHost = true; } io.emit('lobbyUpdate', players); } } });
