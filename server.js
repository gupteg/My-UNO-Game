...
io.on('connection', (socket) => {
  ...
  socket.on('joinGame', ({ playerName, playerId }) => { /* ... (unchanged) ... */ if (gameState && gameState.phase !== 'Lobby' && gameState.phase !== 'GameOver') { let playerToRejoin = null; if (playerId) { playerToRejoin = gameState.players.find(p => p.playerId === playerId && p.status !== 'Active'); } if (playerToRejoin) { console.log(`${playerName} is rejoining as ${playerToRejoin.name}.`); playerToRejoin.status = 'Active'; playerToRejoin.socketId = socket.id; playerToRejoin.name = playerName; if (reconnectTimers[playerToRejoin.playerId]) { clearTimeout(reconnectTimers[playerToRejoin.playerId]); delete reconnectTimers[playerToRejoin.playerId]; } addLog(`Player ${playerToRejoin.name} has reconnected!`); const otherDisconnected = gameState.players.filter(p => p.status === 'Disconnected'); if (otherDisconnected.length === 0 && gameState.isPaused) { gameState.isPaused = false; gameState.pauseInfo = { pauseEndTime: null, pausedForPlayerNames: [] }; addLog("All players reconnected. Game resumed."); } else if (gameState.isPaused) { gameState.pauseInfo.pausedForPlayerNames = otherDisconnected.map(p => p.name); } socket.emit('joinSuccess', { playerId: playerToRejoin.playerId, lobby: players }); io.emit('updateGameState', gameState); return; } else { socket.emit('announce', 'Game is in progress. Cannot join now.'); return; } } let pId = playerId || Math.random().toString(36).substr(2, 9); const existingPlayer = players.find(p => p.playerId === pId); if (existingPlayer) { existingPlayer.socketId = socket.id; existingPlayer.name = playerName; existingPlayer.active = true; } else { const isHost = players.length === 0; players.push({ playerId: pId, socketId: socket.id, name: playerName, isHost: isHost, isReady: isHost, active: true }); } socket.emit('joinSuccess', { playerId: pId, lobby: players }); io.emit('lobbyUpdate', players); });
  ...
  // *** MODIFIED: End Game with Pre-Announce and Nested Timers ***
  socket.on('endGame', () => {
      ...
      if (player && player.isHost) {
          if (gameState && gameState.phase !== 'GameOver') {
              ...
              gameState.phase = 'GameOver'; // Mark as over

              // --- Calculate winner ---
              const finalGamePlayers = [...gameState.players]; // Capture players NOW
              ...
              // --- Timer 1: Delay before showing scores ---
              gameOverToLobbyTimer = setTimeout(() => {
                  if (!gameState) return; // Abort if hard reset occurred
                  io.emit('finalGameOver', gameState); // Now emit scores

                  // --- Timer 2: Delay after showing scores before lobby transition ---
                  scoresToLobbyTimer = setTimeout(() => {
                      if (!gameState && !players.length) return; // Check again for hard reset

                      // Rebuild lobby using captured player data
                      ...
                      // Ensure host exists and is ready
                      ...
                      gameState = null; // Finally, nullify game state
                      io.emit('lobbyUpdate', players); // Send everyone to lobby
                  }, 15000); // 15s after scores appear

              }, 3000); // 3s after winner announcement

          } else if (!gameState) { // If already in lobby
  ...
  socket.on('choosePlayDrawnWild', ({ play, cardIndex }) => {
      ...
      if (play) {
          ...
          // If player had 1 card, drew this Wild (now has 2), and plays it
      ...
      // Only emit update if game didn't end (handleCardPlay might call handleEndOfRound)
  ...
  socket.on('disconnect', () => { /* ... (unchanged, includes host migration fix) ... */ console.log(`Player disconnected: ${socket.id}`); if (gameState && gameState.phase !== 'GameOver') { const disconnectedPlayer = gameState.players.find(p => p.socketId === socket.id); if (disconnectedPlayer && disconnectedPlayer.status === 'Active') { disconnectedPlayer.status = 'Disconnected'; if (gameState.phase !== 'GameOver') { addLog(`Player ${disconnectedPlayer.name} has disconnected. The game is paused.`); gameState.isPaused = true; const pauseEndTime = Date.now() + DISCONNECT_GRACE_PERIOD; const allDisconnected = gameState.players.filter(p => p.status === 'Disconnected'); gameState.pauseInfo = { pauseEndTime: pauseEndTime, pausedForPlayerNames: allDisconnected.map(p => p.name) }; if (reconnectTimers[disconnectedPlayer.playerId]) clearTimeout(reconnectTimers[disconnectedPlayer.playerId]); reconnectTimers[disconnectedPlayer.playerId] = setTimeout(() => handlePlayerRemoval(disconnectedPlayer.playerId), DISCONNECT_GRACE_PERIOD); io.emit('updateGameState', gameState); } } } else { const playerInLobby = players.find(player => player.socketId === socket.id); if (playerInLobby) { playerInLobby.active = false; let hostLeft = false; if (playerInLobby.isHost && players.some(p => p.active)) { playerInLobby.isHost = false; const nextHost = players.find(p => p.active); if (nextHost) { nextHost.isHost = true; nextHost.isReady = true; hostLeft = true; } } if (!players.some(p => p.active)) { players = []; console.log("Last active player left lobby. Clearing lobby."); } io.emit('lobbyUpdate', players); if (hostLeft) console.log(`Host ${playerInLobby.name} disconnected. New host assigned.`); } } });
