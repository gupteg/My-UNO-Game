...
function setupGame(lobbyPlayers) {
    const gamePlayers = lobbyPlayers.map(p => ({
        ...
        unoState: 'safe',
        scoresByRound: [],
        status: 'Active', // Status within the game (Active, Disconnected, Removed)
        // *** MODIFIED: isReady not needed in game state player obj ***
    }));
    ...
    return {
        ...
        readyForNextRound: [], // Used for between rounds
        activeColor: null,
        playDirection: 1,
        drawPenalty: 0,
...
io.on('connection', (socket) => {
  ...
  socket.on('joinGame', ({ playerName, playerId }) => {
    ...
    if (gameState && gameState.phase !== 'Lobby' && gameState.phase !== 'GameOver') {
        ...
        // Simplified rejoin logic (could add name matching if needed)
        if (playerToRejoin) {
            console.log(`${playerName} is rejoining as ${playerToRejoin.name}.`);
            playerToRejoin.status = 'Active';
            playerToRejoin.socketId = socket.id; // Update socket ID
            playerToRejoin.name = playerName; // Update name if changed
            ...
            // Send lobby players array for consistency? No, send full game state.
            socket.emit('joinSuccess', { playerId: playerToRejoin.playerId, lobby: players }); // Send lobby for context initially
            io.emit('updateGameState', gameState); // Send full state
            return;
        } else {
    ...
    if (existingPlayer) {
        ...
        existingPlayer.active = true; // Mark as active in lobby context
        // Keep existing isHost and isReady status
    } else {
  ...
  socket.on('setPlayerReady', () => {
      if (gameState) return; // Only allow in lobby
  ...
  socket.on('kickPlayer', ({ playerIdToKick }) => {
    ...
    if (host) {
        ...
        if (playerToKick) {
            ...
            io.to(playerToKick.socketId).emit('forceDisconnect'); // Force client disconnect
            io.emit('lobbyUpdate', players);
        }
    }
  });
  ...
  socket.on('startGame', ({ password }) => {
      if (gameState) return; // Only allow from lobby
      const host = players.find(p => p.socketId === socket.id && p.isHost);
      if (!host) return;
      ...
      const activePlayers = players.filter(p => p.active); // Check only active players in lobby
      ...
      players = []; // Clear lobby array as game starts
      io.emit('updateGameState', gameState);
  });


  // *** FUNCTION DEFINITION MUST BE WITHIN io.on('connection', ...) SCOPE ***
  ...
  socket.on('endGame', () => {
      ...
      if (player && player.isHost) {
          if (gameState && gameState.phase !== 'GameOver') {
              ...
              io.emit('finalGameOver', gameState); // Broadcast final scores

              // Clear previous timer if exists
              if (gameOverToLobbyTimer) clearTimeout(gameOverToLobbyTimer);

              // Set timer to transition to lobby
              gameOverToLobbyTimer = setTimeout(() => {
                  if (!gameState) return; // Game might have been reset in the meantime

                  // Transform game players back to lobby players
                  players = gameState.players
                      .filter(p => p.status !== 'Removed') // Keep only non-removed players
                      .map(p => ({
                          playerId: p.playerId,
                          socketId: p.socketId,
                          name: p.name,
                          isHost: p.isHost,
                          isReady: p.isHost, // Host true, others false
                          active: p.status !== 'Disconnected' // Active if not disconnected
                      }));

                  gameState = null; // Nullify game state
                  io.emit('lobbyUpdate', players); // Send everyone to lobby
              }, 15000); // 15-second delay

          } else if (!gameState) {
              // If already in lobby, maybe just reset ready status? Or do nothing?
              // For now, let's just ensure ready state is correct
  ...
  socket.on('endSession', () => {
      if (gameState) return; // Only in lobby
      const host = players.find(p => p.socketId === socket.id && p.isHost);
      if (host) {
          console.log(`Host ${host.name} is ending the session.`);
          players.forEach(p => {
              if (p.socketId !== host.socketId) {
                  io.to(p.socketId).emit('forceDisconnect'); // Kick others
              }
          });
          players = [host]; // Keep only host
          host.isReady = true; // Host remains ready
          io.emit('lobbyUpdate', players);
      }
  });

  // *** NEW: Hard Reset Handler ***
  socket.on('hardReset', () => {
      // Allow reset from lobby OR during game if host requests
      ...
      if (host) {
          ...
          // Determine players to disconnect (either from gameState or lobby)
          ...
          players = [{
              ...
              socketId: host.socketId, // Use current socket ID
              name: host.name,
              isHost: true,
              isReady: true,
              active: true
          ...
          io.emit('lobbyUpdate', players); // Update host's view
      }
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (gameState && gameState.phase !== 'GameOver') {
        ...
        if (disconnectedPlayer && disconnectedPlayer.status === 'Active') {
            ...
            // Only add log and pause if game not already over
        ...
        if (playerInLobby) {
            playerInLobby.active = false; // Mark inactive instead of removing immediately
            // players = players.filter(player => player.socketId !== socket.id); // Old removal logic
            let hostLeft = false;
            if (playerInLobby.isHost && players.some(p => p.active)) { // Check if other active players exist
                playerInLobby.isHost = false; // Demote host
                const nextHost = players.find(p => p.active); // Find first available active player
                if (nextHost) {
                    nextHost.isHost = true;
                    hostLeft = true;
                }
            }
             // If only inactive players remain, clear lobby? Or let them time out? Let's keep inactive for now.
            // If the last active player leaves, lobby is effectively empty.
            if (!players.some(p => p.active)) {
                 players = []; // Clear lobby if no one is active
