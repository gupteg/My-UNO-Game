window.addEventListener('DOMContentLoaded', () => {
    ...
    window.expectingLobbyUpdate = false; // Flag for End Game -> Lobby transition
    ...
    // const endSessionBtn = document.getElementById('end-session-btn'); // REMOVED
    ...
    // *** MODIFIED: Removed End Session Listener ***
    ...
    confirmEndYesBtn.addEventListener('click', () => {
        ...
        window.expectingLobbyUpdate = true; // Set flag before emitting
        socket.emit('endGame');
    });

    finalScoreOkBtn.addEventListener('click', () => {
        finalScoreModal.style.display = 'none';
        if (window.expectingLobbyUpdate) {
            window.expectingLobbyUpdate = false; // Reset flag
        } else {
            isGameOver = false;
            gameBoard.style.display = 'none';
            ...
            socket.emit('joinGame', { playerName: playerNameInput.value.trim() || 'Player' });
        }
    });
    ...
    socket.on('connect', () => {
        ...
        if (myPersistentPlayerId) {
            ...
            const savedName = sessionStorage.getItem('unoPlayerName') || playerNameInput.value.trim() || "Player";
            playerNameInput.value = savedName;
            socket.emit('joinGame', { playerName: savedName, playerId: myPersistentPlayerId });
        } else {
    ...
    socket.on('forceDisconnect', () => {
        console.log("Received force disconnect from server.");
        showToast("You have been disconnected by the host.");
        sessionStorage.removeItem('unoPlayerId');
    ...
    function renderLobby(currentLobbyPlayers) {
        ...
        currentLobbyPlayers.forEach(player => {
             if (!player.active) return; // Don't show inactive players
    ...
    function playerHasPlayableNonWildCard(gameState) { if (!gameState || !gameState.players || !gameState.discardPile || gameState.discardPile.length === 0) { return false; } const myPlayer = gameState.players.find(p => p.playerId === myPersistentPlayerId); if (!myPlayer) { return false; } const isMyTurn = gameState.players[gameState.currentPlayerIndex]?.playerId === myPersistentPlayerId; const isPlaying = gameState.phase === 'Playing'; const noPenalty = gameState.drawPenalty === 0 && !gameState.pickUntilState?.active; if (!isMyTurn || !isPlaying || !noPenalty) { return false; } for (const card of myPlayer.hand) { if (card.color !== 'Black') { if (isClientMoveValid(card, gameState)) { return true; } } } return false; }
    ...
    function renderGameLog(logHistory) { if (!gameLogList) return; gameLogList.innerHTML = ''; if (!logHistory) return; const recentLogs = logHistory.slice(0, 8); recentLogs.reverse(); recentLogs.forEach(msg => { const li = document.createElement('li'); li.textContent = msg; gameLogList.prepend(li); }); }
