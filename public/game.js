window.addEventListener('DOMContentLoaded', () => {
    ...
    confirmAfkYesBtn.addEventListener('click', () => {
        if (playerIdToMarkAFK) { socket.emit('markPlayerAFK', { playerIdToMark: playerIdToMarkAFK }); }
        confirmAfkModal.style.display = 'none'; playerIdToMarkAFK = null;
    });
    ...
    imBackBtn.addEventListener('click', () => {
        socket.emit('playerIsBack'); afkNotificationModal.style.display = 'none';
    });
    ...
    confirmEndYesBtn.addEventListener('click', () => {
        confirmEndGameModal.style.display = 'none'; socket.emit('endGame');
    });

    finalScoreOkBtn.addEventListener('click', () => {
        isGameOver = false; finalScoreModal.style.display = 'none'; gameBoard.style.display = 'none';
        lobbyScreen.style.display = 'none'; joinScreen.style.display = 'block';
        sessionStorage.clear(); myPersistentPlayerId = null;
    });

    unoBtn.addEventListener('click', () => {
        socket.emit('callUno'); unoBtn.classList.add('pressed');
        setTimeout(() => unoBtn.classList.remove('pressed'), 300);
    });

    nextRoundBtn.addEventListener('click', () => {
        socket.emit('playerReadyForNextRound'); nextRoundBtn.disabled = true; nextRoundBtn.textContent = 'Waiting...';
    });

    nextRoundOkBtn.addEventListener('click', () => {
        socket.emit('playerReadyForNextRound'); endOfRoundDiv.style.display = 'none';
    });

    dealCardsBtn.addEventListener('click', () => {
        const numCards = dealCardsInput.value; socket.emit('dealChoice', { numCards });
    });

    colorPickerModal.addEventListener('click', (event) => {
        if (event.target.matches('.color-btn')) {
            const color = event.target.dataset.color; socket.emit('colorChosen', { color });
        }
    });

    drawnWildModal.addEventListener('click', (event) => {
        const cardIndex = parseInt(drawnWildModal.dataset.cardIndex);
        if (event.target.id === 'option-play-wild') { socket.emit('choosePlayDrawnWild', { play: true, cardIndex }); }
        else if (event.target.id === 'option-keep-wild') { socket.emit('choosePlayDrawnWild', { play: false, cardIndex }); }
    ...
    swapModal.addEventListener('click', (event) => {
        if (event.target.matches('.player-swap-btn')) {
            const targetPlayerId = event.target.dataset.playerId; socket.emit('swapHandsChoice', { targetPlayerId });
    ...
    arrangeHandBtn.addEventListener('click', () => {
        ...
        myPlayer.hand = sortedHand; socket.emit('rearrangeHand', { newHand: sortedHand }); displayGame(window.gameState);
    });
    ...
    socket.on('connect', () => {
        ...
        if (myPersistentPlayerId) { const playerName = playerNameInput.value.trim() || "Player"; socket.emit('joinGame', { playerName, playerId: myPersistentPlayerId }); }
    });

    socket.on('joinSuccess', ({ playerId, lobby }) => {
        myPersistentPlayerId = playerId; sessionStorage.setItem('unoPlayerId', playerId); renderLobby(lobby);
    });

    socket.on('lobbyUpdate', (players) => { if (!isGameOver) renderLobby(players); });

    socket.on('updateGameState', (gameState) => {
        if (gameState.roundOver) { displayGame(gameState); }
        else { joinScreen.style.display = 'none'; lobbyScreen.style.display = 'none'; endOfRoundDiv.style.display = 'none'; gameBoard.style.display = 'flex'; displayGame(gameState); }
    });

    socket.on('announceRoundWinner', ({ winnerNames }) => {
        let message = `${winnerNames} wins the round!`; if (winnerNames.includes(' and ')) { message = `${winnerNames} win the round!`; } showUnoAnnouncement(message);
    });
    ...
    socket.on('roundOver', ({ winnerName, scores, finalGameState }) => {
        setTimeout(() => {
            ...
            const scoresDisplay = document.getElementById('scores-display'); scoresDisplay.innerHTML = '<h3>Round Scores</h3>';
            const scoreTable = document.createElement('table'); scoreTable.className = 'score-table'; let tableHTML = '<thead><tr><th>Player</th><th>Hand Score</th><th>Total Score</th></tr></thead><tbody>';
            const sortedPlayers = [...finalGameState.players].sort((a,b) => a.score - b.score);
            sortedPlayers.forEach(p => { const roundScoreForPlayer = p.scoresByRound[p.scoresByRound.length - 1]; const displayScore = roundScoreForPlayer !== undefined ? roundScoreForPlayer : '-'; const isWinner = winnerName.includes(p.name); tableHTML += `<tr class="${isWinner ? 'winner-row' : ''}"><td>${p.name} ${p.status === 'Removed' ? '(Removed)' : ''}</td><td>${displayScore}</td><td>${p.score}</td></tr>`; });
            tableHTML += '</tbody>'; scoreTable.innerHTML = tableHTML; scoresDisplay.appendChild(scoreTable);
            const myPlayer = finalGameState.players.find(p => p.playerId === myPersistentPlayerId);
            if (myPlayer && myPlayer.isHost && myPlayer.status !== 'Removed') { hostRoundEndControls.style.display = 'flex'; nextRoundOkBtn.style.display = 'none'; nextRoundBtn.disabled = false; nextRoundBtn.textContent = 'Start Next Round'; }
            else if (myPlayer && myPlayer.status !== 'Removed') { hostRoundEndControls.style.display = 'none'; nextRoundOkBtn.style.display = 'block'; }
            else { hostRoundEndControls.style.display = 'none'; nextRoundOkBtn.style.display = 'none'; }
            endOfRoundDiv.style.display = 'flex';
        }, 2000);
    });

    socket.on('finalGameOver', (finalGameState) => {
        isGameOver = true; gameBoard.style.display = 'none'; endOfRoundDiv.style.display = 'none';
        renderFinalScores(finalGameState); finalScoreModal.style.display = 'flex';
    });

    socket.on('drawnWildCard', ({ cardIndex, drawnCard }) => {
        if (window.gameState) { const myPlayer = window.gameState.players.find(p => p.playerId === myPersistentPlayerId); if (myPlayer && !myPlayer.hand.some((c, idx) => idx === cardIndex)) { if(drawnCard && drawnCard.color && drawnCard.value) { myPlayer.hand.splice(cardIndex, 0, drawnCard); displayGame(window.gameState); } else { console.error("Invalid drawnWildCard data:", drawnCard); } } }
        drawnWildModal.dataset.cardIndex = cardIndex; drawnWildModal.style.display = 'flex';
    });
    ...
    // *** Listener for Discarded Wilds Modal ***
    socket.on('showDiscardedWildsModal', (discardedInfo) => {
        if (!discardedInfo || discardedInfo.length === 0) return;
        discardedWildsList.innerHTML = '';
        discardedInfo.forEach(playerInfo => {
            const sectionDiv = document.createElement('div'); sectionDiv.className = 'discarded-wilds-player-section';
            const nameHeader = document.createElement('h4'); nameHeader.textContent = `${playerInfo.playerName} discarded:`; sectionDiv.appendChild(nameHeader);
            const cardsContainer = document.createElement('div'); cardsContainer.className = 'card-container';
            if (playerInfo.cards && playerInfo.cards.length > 0) { playerInfo.cards.forEach(card => { const cardEl = createCardElement(card, -1); cardsContainer.appendChild(cardEl); }); }
            else { cardsContainer.textContent = 'No wild cards.'; }
            sectionDiv.appendChild(cardsContainer); discardedWildsList.appendChild(sectionDiv);
        });
    ...
    function renderLobby(players) { /* ... (unchanged) ... */
        const me = players.find(p => p.playerId === myPersistentPlayerId); if (!me) { showToast("You have been kicked from the lobby."); sessionStorage.clear(); myPersistentPlayerId = null; setTimeout(() => { location.reload(); }, 1500); return; } joinScreen.style.display = 'none'; lobbyScreen.style.display = 'block'; gameBoard.style.display = 'none'; endOfRoundDiv.style.display = 'none'; finalScoreModal.style.display = 'none'; playerList.innerHTML = ''; if (gameLogList) gameLogList.innerHTML = ''; let amIHost = me.isHost; players.forEach(player => { const playerItem = document.createElement('li'); const nameSpan = document.createElement('span'); let content = player.name; if (player.isHost) content += ' 👑 (Host)'; if (player.playerId === myPersistentPlayerId) content += ' (You)'; nameSpan.textContent = content; playerItem.appendChild(nameSpan); if (amIHost && player.playerId !== myPersistentPlayerId) { const kickBtn = document.createElement('button'); kickBtn.className = 'kick-btn'; kickBtn.textContent = 'Kick'; kickBtn.dataset.playerId = player.playerId; playerItem.appendChild(kickBtn); } playerList.appendChild(playerItem); }); startGameBtn.style.display = (amIHost && players.length >= 2) ? 'block' : 'none'; hostMessage.style.display = amIHost ? 'none' : 'block';
    }
    ...
    function showUnoAnnouncement(message) { /* ... (unchanged) ... */ unoAnnouncementText.textContent = message; if (message.length > 15) { unoAnnouncementText.style.fontSize = '8vw'; } else if (message.length > 10) { unoAnnouncementText.style.fontSize = '10vw'; } else { unoAnnouncementText.style.fontSize = '15vw'; } unoAnnouncementOverlay.classList.add('show'); setTimeout(() => { unoAnnouncementOverlay.classList.remove('show'); }, 1900); }
    ...
    function animateCardPlay(playerId, card, cardIndex) { /* ... (unchanged) ... */ const discardPileEl = document.querySelector('#discard-pile-dropzone .card'); const playerAreaEl = document.querySelector(`[data-player-id="${playerId}"]`); if (!discardPileEl || !playerAreaEl) return; const startRect = playerAreaEl.getBoundingClientRect(); const endRect = discardPileEl.getBoundingClientRect(); const boardRect = gameBoard.getBoundingClientRect(); const clone = createCardElement(card, -1); clone.classList.add('flying-card'); clone.style.top = `${startRect.top - boardRect.top + (startRect.height / 2) - 60}px`; clone.style.left = `${startRect.left - boardRect.left + (startRect.width / 2) - 40}px`; clone.style.width = '80px'; clone.style.height = '120px'; if (playerId === myPersistentPlayerId && window.gameState) { const myPlayer = window.gameState.players.find(p => p.playerId === myPersistentPlayerId); if (myPlayer) { const cardToHide = playerAreaEl.querySelector(`.card[data-card-index="${cardIndex}"]`); if(cardToHide) cardToHide.style.visibility = 'hidden'; else console.warn(`Could not find card element at index ${cardIndex} to hide.`); } } gameBoard.appendChild(clone); requestAnimationFrame(() => { clone.style.top = `${endRect.top - boardRect.top}px`; clone.style.left = `${endRect.left - boardRect.left}px`; clone.style.transform = `rotate(360deg)`; clone.style.width = `${endRect.width}px`; clone.style.height = `${endRect.height}px`; }); setTimeout(() => { clone.remove(); }, 800); }
    ...
    function displayGame(gameState) {
        ...
        // Render player hands FIRST, as it now includes direction arrows
        renderPlayers(gameState);
        // Then render piles
        renderPiles(gameState);

        const myPlayer = gameState.players.find(p => p.playerId === myPersistentPlayerId);
        if (!myPlayer) { showToast("Removed from game."); sessionStorage.clear(); setTimeout(() => location.reload(), 1500); return; }
        ...
        if (unoBtn) { /* ... (uno button logic unchanged) ... */ }
        if (drawCardBtn) { /* ... (draw button logic unchanged) ... */ }

        // *** REMOVED old direction indicator update logic ***
    ...
    function renderPlayers(gameState) {
        ...
        gameState.players.forEach((player, playerIndex) => {
            ...
            if (player.status === 'Disconnected') playerArea.classList.add('disconnected');
            else if (player.status === 'Removed') playerArea.classList.add('disconnected', 'removed');
            ...
            if (player.unoState === 'unsafe') playerArea.classList.add('uno-unsafe');
            else if (player.unoState === 'declared' && player.playerId === myPersistentPlayerId) playerArea.classList.add('uno-declared');
            if (player.hand.length === 1 && !gameState.roundOver) playerArea.classList.add('has-uno');

            const playerInfo = document.createElement('div');
            playerInfo.className = 'player-info';

            // Player Name and Score Span (allows ellipsis)
            const nameSpan = document.createElement('span');
            nameSpan.className = 'player-name-info'; // New class for flex growth
            ...
            // *** NEW: Direction Arrow Span ***
            const arrowSpan = document.createElement('span');
            arrowSpan.className = 'direction-arrow';
            arrowSpan.textContent = gameState.playDirection === 1 ? '→' : '←';
            playerInfo.appendChild(arrowSpan);

            // AFK Button (if applicable)
            if (isHost && player.playerId !== myPersistentPlayerId && player.status === 'Active' && !gameState.roundOver) {
                ...
                playerInfo.appendChild(afkBtn); // Appends after name and arrow due to order
            }
            ...
            // --- Hand Rendering (unchanged logic) ---
            if (player.playerId === myPersistentPlayerId) {
                 const currentHand = player.hand;
                 currentHand.forEach((card, indexInHand) => { /* ... create cardEl, add listeners, append ... */
                    const originalCardIndex = indexInHand; const cardEl = createCardElement(card, originalCardIndex); const isMyTurn = isCurrentPlayer; if (isMyTurn && !gameState.isPaused && !gameState.roundOver && player.status === 'Active') cardEl.classList.add('clickable'); cardEl.addEventListener('click', () => { if (isMyTurn && !gameState.isPaused && !gameState.roundOver && player.status === 'Active') { if (isClientMoveValid(card, gameState)) socket.emit('playCard', { cardIndex: originalCardIndex }); else triggerInvalidMoveFeedback(cardEl); } }); cardEl.draggable = isMyTurn && !gameState.isPaused && !gameState.roundOver; cardContainer.appendChild(cardEl);
                 });
                 // Drag and Drop listeners (unchanged logic using on-event handlers)
                 cardContainer.ondragstart = e => { /* ... */ }; cardContainer.ondragend = e => { /* ... */ }; cardContainer.ondragover = e => { /* ... */ };
             } else { // Other players
                if (gameState.roundOver && player.status !== 'Removed') { player.hand.forEach((card, cardIndex) => { const cardEl = createCardElement(card, cardIndex); cardContainer.appendChild(cardEl); }); }
                else { if (player.hand.length === 1) { const cardEl = document.createElement('div'); cardEl.className = 'card uno-warning'; const unoSpan = document.createElement('span'); unoSpan.textContent = 'UNO'; cardEl.appendChild(unoSpan); cardContainer.appendChild(cardEl); } else { for (let j = 0; j < player.hand.length; j++) { const cardEl = document.createElement('div'); cardEl.className = 'card card-back'; cardContainer.appendChild(cardEl); } } }
             }
            // --- End Hand Rendering ---
