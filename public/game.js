window.addEventListener('DOMContentLoaded', () => {
    ...
    // *** NEW: Discarded Wilds Modal Refs ***
    ...
    pickUntilModal.addEventListener('click', (event) => {
        let choice = null;
        if (event.target.id === 'option-pick-color') { choice = 'pick-color'; }
        else if (event.target.id === 'option-discard-wilds') { choice = 'discard-wilds'; }
        if (choice) { socket.emit('pickUntilChoice', { choice }); }
    ...
    arrangeHandBtn.addEventListener('click', () => {
        ...
        myPlayer.hand = sortedHand; // Optimistic update for immediate visual feedback
        socket.emit('rearrangeHand', { newHand: sortedHand });
        displayGame(window.gameState); // Re-render my hand
    });
    ...
    showDiscardPileBtn.addEventListener('click', () => {
        ...
        if (lastTenDiscards.length === 0) {
            ...
            lastTenDiscards.forEach(item => {
                ...
                playerP.textContent = `Played by: ${item.playerName || 'Unknown'}`; // Added fallback
                if (item.card) {
                ...
                } else { console.warn("Discard pile item missing card data:", item); }
            });
        }
    ...
    // *** NEW: Discarded Wilds Modal OK Listener ***
    discardedWildsOkBtn.addEventListener('click', () => {
            discardedWildsModal.style.display = 'none';
    });
    ...
    socket.on('updateGameState', (gameState) => {
        if (gameState.roundOver) {
            displayGame(gameState); // Render state even if round is over (shows final hands)
        } else {
            joinScreen.style.display = 'none';
            lobbyScreen.style.display = 'none';
            endOfRoundDiv.style.display = 'none'; // Hide round end modal if new round starts
    ...
    socket.on('roundOver', ({ winnerName, scores, finalGameState }) => {
        setTimeout(() => {
            displayGame(finalGameState); // Refresh state before showing modal
            ...
            // Sort players by score for display in the table
             const sortedPlayers = [...finalGameState.players].sort((a,b) => a.score - b.score);
            sortedPlayers.forEach(p => {
                const roundScoreForPlayer = p.scoresByRound[p.scoresByRound.length - 1];
                const displayScore = roundScoreForPlayer !== undefined ? roundScoreForPlayer : '-'; // Handle potential undefined score
                const isWinner = winnerName.includes(p.name);
                tableHTML += `<tr class="${isWinner ? 'winner-row' : ''}"><td>${p.name} ${p.status === 'Removed' ? '(Removed)' : ''}</td><td>${displayScore}</td><td>${p.score}</td></tr>`;
            });
            tableHTML += '</tbody>';
            ...
            if (myPlayer && myPlayer.isHost && myPlayer.status !== 'Removed') { // Host must not be removed
                hostRoundEndControls.style.display = 'flex';
                nextRoundOkBtn.style.display = 'none';
                nextRoundBtn.disabled = false;
                nextRoundBtn.textContent = 'Start Next Round';
            } else if (myPlayer && myPlayer.status !== 'Removed') { // Active non-host
                hostRoundEndControls.style.display = 'none';
                nextRoundOkBtn.style.display = 'block';
            } else { // Spectator (Removed or somehow not found)
                hostRoundEndControls.style.display = 'none';
                nextRoundOkBtn.style.display = 'none'; // Cannot proceed
            }
    ...
    socket.on('drawnWildCard', ({ cardIndex, drawnCard }) => {
        // Optimistically add card if gameState exists, otherwise wait for update
        if (window.gameState) {
            const myPlayer = window.gameState.players.find(p => p.playerId === myPersistentPlayerId);
            if (myPlayer && !myPlayer.hand.some((c, idx) => idx === cardIndex)) { // Avoid adding duplicate if update was fast
                 // Check if the card object is valid before pushing
                if(drawnCard && drawnCard.color && drawnCard.value) {
                    myPlayer.hand.splice(cardIndex, 0, drawnCard); // Insert at correct index
                    displayGame(window.gameState); // Re-render hand
                } else {
                     console.error("Received invalid drawnWildCard data:", drawnCard);
                }
            }
        }
        drawnWildModal.dataset.cardIndex = cardIndex; // Still show modal
    ...
    socket.on('animatePlay', ({ playerId, card, cardIndex }) => { animateCardPlay(playerId, card, cardIndex); });
     ...
     // *** NEW: Listener for Discarded Wilds Modal ***
    socket.on('showDiscardedWildsModal', (discardedInfo) => {
            if (!discardedInfo || discardedInfo.length === 0) return;

            discardedWildsList.innerHTML = ''; // Clear previous content

            discardedInfo.forEach(playerInfo => {
                const sectionDiv = document.createElement('div');
                sectionDiv.className = 'discarded-wilds-player-section';

                const nameHeader = document.createElement('h4');
                nameHeader.textContent = `${playerInfo.playerName} discarded:`;
                sectionDiv.appendChild(nameHeader);

                const cardsContainer = document.createElement('div');
                cardsContainer.className = 'card-container';
                if (playerInfo.cards && playerInfo.cards.length > 0) {
                     playerInfo.cards.forEach(card => {
                        const cardEl = createCardElement(card, -1); // Use your existing function
                        cardsContainer.appendChild(cardEl);
                    });
                } else {
                     // Optionally display text if no cards were discarded by this player (shouldn't happen with current server logic)
                     cardsContainer.textContent = 'No wild cards.';
                }

                sectionDiv.appendChild(cardsContainer);
                discardedWildsList.appendChild(sectionDiv);
            });

            discardedWildsModal.style.display = 'flex';
        });
    ...
    function renderLobby(players) {
        ...
        if (!me) { /* ... (kick logic unchanged) ... */ return; }
        joinScreen.style.display = 'none';
        ...
        players.forEach(player => { /* ... (lobby rendering unchanged) ... */
        ...
        startGameBtn.style.display = (amIHost && players.length >= 2) ? 'block' : 'none';
    ...
    function showUnoAnnouncement(message) { /* ... (unchanged) ... */
        unoAnnouncementText.textContent = message;
        if (message.length > 15) { unoAnnouncementText.style.fontSize = '8vw'; } // Adjust threshold maybe
        else if (message.length > 10) { unoAnnouncementText.style.fontSize = '10vw'; }
        else { unoAnnouncementText.style.fontSize = '15vw'; }
        unoAnnouncementOverlay.classList.add('show');
        setTimeout(() => { unoAnnouncementOverlay.classList.remove('show'); }, 1900);
     }

    function isClientMoveValid(playedCard, gameState) { /* ... (logic unchanged, uses topDiscard.card) ... */
        if (!gameState || !gameState.discardPile || gameState.discardPile.length === 0) return false;
        const topDiscard = gameState.discardPile[0];
        if (!topDiscard || !topDiscard.card) return false;
        const topCard = topDiscard.card;
    ...
    function triggerInvalidMoveFeedback(cardElement) { /* ... (unchanged) ... */
        ...
        setTimeout(() => { cardElement.classList.remove('invalid-shake'); }, 500);
        setTimeout(() => { invalidMoveCallout.classList.remove('show'); }, 1500);
    }

    function animateCardPlay(playerId, card, cardIndex) { /* ... (unchanged) ... */
        ...
        clone.style.width = '80px'; clone.style.height = '120px';
        if (playerId === myPersistentPlayerId && window.gameState) {
             const myPlayer = window.gameState.players.find(p => p.playerId === myPersistentPlayerId);
             if (myPlayer) {
                // Try to find the *element* corresponding to the index being played
                ...
                else console.warn(`Could not find card element at index ${cardIndex} to hide during animation.`);
             }
        }
    ...
    function animateCardDraw(playerId, count) { /* ... (unchanged) ... */
        ...
        const boardRect = gameBoard.getBoundingClientRect();
        const smallCardWidth = 80; const scaleFactor = smallCardWidth / startRect.width;
        for (let i = 0; i < count; i++) {
            ...
            cardBack.style.width = `${startRect.width}px`; cardBack.style.height = `${startRect.height}px`;
            ...
            setTimeout(() => {
                requestAnimationFrame(() => {
                    ...
                    cardBack.style.top = top; cardBack.style.left = left;
                    cardBack.style.width = `${smallCardWidth}px`; cardBack.style.height = `${smallCardWidth * 1.5}px`;
                });
            }, i * 100 + 50);
            setTimeout(() => { cardBack.remove(); }, 800 + (i * 100));
        }
    }
    ...
    function animateHandSwap(p1_id, p2_id) { /* ... (unchanged) ... */
        ...
        const animateHand = (cards, toArea) => {
            ...
            cards.forEach(card => {
                ...
                clone.classList.add('flying-card');
                clone.style.top = `${startRect.top - boardRect.top}px`; clone.style.left = `${startRect.left - boardRect.left}px`;
                gameBoard.appendChild(clone); clones.push(clone); card.style.visibility = 'hidden';
            });
            clones.forEach((clone, i) => {
                setTimeout(() => { requestAnimationFrame(() => {
                        const top = `${endRect.top - boardRect.top + 10}px`; const left = `${endRect.left - boardRect.left + (i * 20)}px`;
                        clone.style.top = top; clone.style.left = left; });
                }, i * 50); setTimeout(() => clone.remove(), 800 + (i*50));
            });
        };
        animateHand(p1_cards, p2_area); animateHand(p2_cards, p1_area);
    }

    function addMessageToGameLog(message) { /* ... (unchanged) ... */
        if (!gameLogList) return;
        const li = document.createElement('li'); li.textContent = message;
        gameLogList.prepend(li);
        while (gameLogList.children.length > 8) { gameLogList.lastChild.remove(); }
    }

    function renderFinalScores(finalGameState) { /* ... (logic unchanged, includes check for active/disconnected winners) ... */
         const players = finalGameState.players;
        const numRounds = finalGameState.roundNumber;
        const table = document.createElement('table'); table.className = 'score-table final-table';
        let headerHtml = '<thead><tr><th>Round</th>'; players.forEach(p => { headerHtml += `<th>${p.name}</th>`; }); headerHtml += '</tr></thead>';
        let bodyHtml = '<tbody>';
        for (let i = 0; i < numRounds; i++) {
            bodyHtml += `<tr><td>${i + 1}</td>`;
            players.forEach(p => { const score = p.scoresByRound[i] !== undefined ? p.scoresByRound[i] : '-'; bodyHtml += `<td>${score}</td>`; });
            bodyHtml += '</tr>';
        } bodyHtml += '</tbody>';
        let footerHtml = '<tfoot><tr><td><strong>Total</strong></td>'; let lowestScore = Infinity;
        players.forEach(p => { if (p.status === 'Active' || p.status === 'Disconnected') { if (p.score < lowestScore) lowestScore = p.score; } footerHtml += `<td><strong>${p.score}</strong></td>`; });
        footerHtml += '</tr></tfoot>'; table.innerHTML = headerHtml + bodyHtml + footerHtml;
        finalScoreTableContainer.innerHTML = ''; finalScoreTableContainer.appendChild(table);
    ...
    function createCardElement(card, cardIndex) { /* ... (unchanged, includes safety check) ... */
        const cardDiv = document.createElement('div');
        if (!card || !card.color || !card.value) { console.error("Invalid card data:", card); cardDiv.className = 'card Black'; cardDiv.textContent = '?'; return cardDiv; }
        cardDiv.className = `card ${card.color}`; cardDiv.dataset.cardIndex = cardIndex;
        if (!isNaN(card.value)) {
            const numberSpan = document.createElement('span'); numberSpan.className = 'number-circle'; numberSpan.textContent = card.value; cardDiv.appendChild(numberSpan);
        } else {
            const actionSpan = document.createElement('span'); actionSpan.className = 'action-text'; actionSpan.innerHTML = card.value.replace(/\s/g, '<br>'); cardDiv.appendChild(actionSpan);
        } return cardDiv;
    }

    function makeDraggable(element) { /* ... (logic unchanged) ... */
        let pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0;
        const header = element.querySelector('.modal-content h3, .modal-content h2, .modal-content p');
        function dragMouseDown(e) { e.preventDefault(); pos3 = e.clientX; pos4 = e.clientY; document.onmouseup = closeDragElement; document.onmousemove = elementDrag; }
        function touchDown(e) { pos3 = e.touches[0].clientX; pos4 = e.touches[0].clientY; document.ontouchend = closeDragElement; document.ontouchmove = elementTouchDrag; }
        function elementDrag(e) { e.preventDefault(); pos1 = pos3 - e.clientX; pos2 = pos4 - e.clientY; pos3 = e.clientX; pos4 = e.clientY; let newTop = element.offsetTop - pos2; let newLeft = element.offsetLeft - pos1; element.style.top = newTop + "px"; element.style.left = newLeft + "px"; }
        function elementTouchDrag(e) { e.preventDefault(); pos1 = pos3 - e.touches[0].clientX; pos2 = pos4 - e.touches[0].clientY; pos3 = e.touches[0].clientX; pos4 = e.touches[0].clientY; let newTop = element.offsetTop - pos2; let newLeft = element.offsetLeft - pos1; element.style.top = newTop + "px"; element.style.left = newLeft + "px"; }
        ...
        if (header) { header.style.cursor = 'move'; header.onmousedown = dragMouseDown; header.ontouchstart = touchDown; }
        else { const content = element.querySelector('.modal-content'); if (content) { content.style.cursor = 'move'; content.onmousedown = dragMouseDown; content.ontouchstart = touchDown; } }
     }
    ...
    function displayGame(gameState) {
        ...
        if (!myPlayer) { /* ... (kick/removal failsafe) ... */ return; }

        const currentPlayer = gameState.players[gameState.currentPlayerIndex]; // Defined early for reuse
        ...
        if (actionBar) { /* ... (action bar logic unchanged) ... */
            if (isPaused && gameState.pauseInfo && gameState.pauseInfo.pauseEndTime) { const { pauseEndTime, pausedForPlayerNames } = gameState.pauseInfo; const names = pausedForPlayerNames.join(', '); const updateTimer = () => { const remaining = Math.max(0, Math.floor((pauseEndTime - Date.now()) / 1000)); actionBar.textContent = `Waiting ${remaining}s for ${names} to rejoin...`; }; updateTimer(); countdownInterval = setInterval(updateTimer, 1000); }
             else if (gameState.roundOver) { const host = gameState.players.find(p => p.isHost); const hostIsReady = gameState.readyForNextRound.includes(host?.playerId); const activePlayers = gameState.players.filter(p => p.status === 'Active'); const allReady = activePlayers.every(p => gameState.readyForNextRound.includes(p.playerId)); if (hostIsReady && !allReady) { const waitingOnPlayers = activePlayers.filter(p => !gameState.readyForNextRound.includes(p.playerId)); const waitingOnNames = waitingOnPlayers.map(p => p.name).join(', '); actionBar.textContent = `Waiting for ${waitingOnNames} to click OK...`; } else { actionBar.textContent = `Round Over! Waiting for players to start the next round.`; } }
             else if (gameState.needsDealChoice) { const dealer = gameState.players.find(p => p.playerId === gameState.needsDealChoice); actionBar.textContent = dealer ? `Waiting for ${dealer.name} to deal...` : 'Waiting for dealer...'; }
             else if (gameState.needsColorChoice) { const chooser = gameState.players.find(p => p.playerId === gameState.needsColorChoice); actionBar.textContent = chooser ? `${chooser.name} is choosing a color...` : 'Choosing a color...'; }
             else if (currentPlayer) { if (currentPlayer.status === 'Active') { actionBar.textContent = `Waiting for ${currentPlayer.name} to play...`; } else { actionBar.textContent = `Waiting for ${currentPlayer.name} to reconnect...`; } }
             else { actionBar.textContent = "Game is starting..."; }
         }
        ...
        if (unoBtn) { /* ... (logic unchanged) ... */
            if (gameState.activeColor && gameState.activeColor !== 'Black') { const colorMap = { "Red": "#ff5555", "Green": "#55aa55", "Blue": "#5555ff", "Yellow": "#ffaa00" }; unoBtn.style.backgroundColor = colorMap[gameState.activeColor]; } else { unoBtn.style.backgroundColor = '#333'; }
            unoBtn.disabled = !(myPlayer && myPlayer.hand.length === 2 && isMyTurn && !isPaused && !gameState.roundOver); // Enable only if 2 cards AND my turn
            unoBtn.classList.toggle('uno-ready', !unoBtn.disabled);
         }

        if (drawCardBtn) { /* ... (logic unchanged, uses full name) ... */
            if (currentPlayer) { const pickUntilInfo = gameState.pickUntilState; const isPickUntilActive = pickUntilInfo?.active && pickUntilInfo.targetPlayerIndex === gameState.currentPlayerIndex; if (isPickUntilActive) { drawCardBtn.textContent = `${currentPlayer.name} PICKS FOR ${pickUntilInfo.targetColor.toUpperCase()}`; } else if (gameState.drawPenalty > 0) { drawCardBtn.textContent = `${currentPlayer.name} DRAWS ${gameState.drawPenalty}`; } else { drawCardBtn.textContent = 'DRAW CARD'; } drawCardBtn.disabled = !isMyTurn || isPaused || gameState.roundOver; }
            else { drawCardBtn.textContent = 'DRAW CARD'; drawCardBtn.disabled = true; }
         }

        // *** NEW: Update Direction Indicator ***
        const directionIndicatorEl = document.getElementById('direction-indicator');
        if (directionIndicatorEl) {
            if (gameState.playDirection === 1) {
                directionIndicatorEl.textContent = '↻';
                directionIndicatorEl.title = 'Clockwise';
            } else if (gameState.playDirection === -1) {
                directionIndicatorEl.textContent = '↺';
                directionIndicatorEl.title = 'Counter-Clockwise';
            } else {
                 directionIndicatorEl.textContent = ''; directionIndicatorEl.title = '';
            }
        }
        // --- End Update Direction Indicator ---
    ...
    makeDraggable(document.getElementById('discarded-wilds-modal')); // *** NEW ***
