window.addEventListener('DOMContentLoaded', () => {
    ...
    // *** MODIFIED: Start Game includes password ***
    ...
    // *** NEW: Host Control Buttons ***
    endSessionBtn.addEventListener('click', () => {
        socket.emit('endSession'); // Add confirmation later if needed
    });
    ...
    // *** MODIFIED: Checks flag before potentially rejoining ***
    finalScoreOkBtn.addEventListener('click', () => {
        finalScoreModal.style.display = 'none';
        if (window.expectingLobbyUpdate) {
            // Just hide modal, wait for lobbyUpdate event
            window.expectingLobbyUpdate = false; // Reset flag
        } else {
            // Natural game over - return to lobby
            isGameOver = false;
            gameBoard.style.display = 'none';
            lobbyScreen.style.display = 'block';
            sessionStorage.removeItem('unoPlayerId'); // Clear only ID, keep name
            myPersistentPlayerId = null;
            socket.emit('joinGame', { playerName: playerNameInput.value.trim() || 'Player' }); // Rejoin lobby
        }
    });
    ...
    socket.on('connect', () => {
        ...
        if (myPersistentPlayerId) {
            ...
            playerNameInput.value = savedName; // Pre-fill name field
    ...
    socket.on('joinSuccess', ({ playerId, lobby }) => {
        ...
        if (me) {
            sessionStorage.setItem('unoPlayerName', me.name); // Store name on successful join/rejoin
            playerNameInput.value = me.name;
        }
        renderLobby(lobby); // Render lobby on successful join
    });

    socket.on('lobbyUpdate', (currentLobbyPlayers) => {
        console.log('Received lobbyUpdate from server.');
        if (isGameOver) return; // Don't render lobby if game just ended (wait for final score)

        // Only render lobby if we are not currently in an active game
        if (!window.gameState || window.gameState.phase === 'Lobby' || window.gameState.phase === 'GameOver') {
            ...
            finalScoreModal.style.display = 'none';
            window.gameState = null; // Ensure game state is cleared when back in lobby
        }
    });

    // *** NEW: Force Disconnect Handler ***
    socket.on('forceDisconnect', () => {
        ...
        sessionStorage.removeItem('unoPlayerName'); // Clear name too
        myPersistentPlayerId = null;
        setTimeout(() => {
            location.reload(); // Reload page to go back to join screen
        }, 1500);
    });


    socket.on('updateGameState', (gameState) => { window.gameState = gameState; if (gameState.phase === 'Lobby') { console.warn("Received gameState update with phase 'Lobby', rendering lobby."); renderLobby(gameState.players); joinScreen.style.display = 'none'; gameBoard.style.display = 'none'; lobbyScreen.style.display = 'block'; } else if (gameState.phase === 'GameOver') { /* Do nothing - finalGameOver handles this */ } else { joinScreen.style.display = 'none'; lobbyScreen.style.display = 'none'; gameBoard.style.display = 'flex'; displayGame(gameState); } });
    ...
    // *** MODIFIED: renderLobby handles ready status and host controls ***
    function renderLobby(currentLobbyPlayers) {
        const me = currentLobbyPlayers.find(p => p.playerId === myPersistentPlayerId);
        // If 'me' is not found but we have a stored ID, means we were kicked or session ended.
        ...
        if (!me) { // If still no 'me', likely fresh join failed, stay on join screen
        ...
        endOfRoundDiv.style.display = 'none';
        finalScoreModal.style.display = 'none';
        if (gameLogList) gameLogList.innerHTML = ''; // Clear logs when back in lobby

        playerList.innerHTML = '';
        let amIHost = me.isHost;

        currentLobbyPlayers.forEach(player => {
             // Only render active players in the lobby list
        ...
        if (amIHost) {
            ...
            // Enable/Disable Start Game button based on conditions
    ...
    function renderGameLog(logHistory) { /* ... (unchanged, uses gameState.gameLog) ... */ if (!gameLogList) return; gameLogList.innerHTML = ''; if (!logHistory) return; const recentLogs = logHistory.slice(0, 8); recentLogs.reverse(); recentLogs.forEach(msg => { const li = document.createElement('li'); li.textContent = msg; gameLogList.prepend(li); }); }
    ...
    makeDraggable(document.getElementById('confirm-hard-reset-modal')); // Make new modal draggable
